...
class Admin extends CI_Controller
{
    ...
    function osadStudRept($param1 = '')
    {
        ...
        $page_data['osadStudent'] = $this->db->get(
            'osad_student',
            array(
                'id' => $param1
            )
        )->result_array();
        $page_data['acdSession'] = $this->db->get(
            'acd_session',
            array(
                'is_open' => '1'
            )
        )->result_array();
        $page_data['osadacdhistory'] = $this->db->get_where(
            'osad_acd_history',
            array(
    ...
    function noticeboard($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'create') {
            ...
            foreach ($admins as $row) {
                $noticboard_notification_admin_data['admin_id'] = $row['admin_id'];
                $noticboard_notification_admin_data['notice_id'] = $noticeboard_id;
                $this->db->insert('noticeboard_notification', $noticboard_notification_admin_data);
            }

            // create notification for teachers
            foreach ($teachers as $row) {
                $noticboard_notification_teacher_data['teacher_id'] = $row['teacher_id'];
                $noticboard_notification_teacher_data['notice_id'] = $noticeboard_id;
                $this->db->insert('noticeboard_notification', $noticboard_notification_teacher_data);
            }

            // create notification for teachers
            foreach ($parents as $row) {
                $noticboard_notification_parent_data['parent_id'] = $row['parent_id'];
                $noticboard_notification_parent_data['notice_id'] = $noticeboard_id;
                $this->db->insert('noticeboard_notification', $noticboard_notification_parent_data);
            }
        ...
        if ($param1 == 'delete') {
            ...
            $this->db->delete('noticeboard_notification');
            $this->session->set_flashdata('flash_message', get_phrase('data_deleted'));
            redirect(base_url() . 'index.php?admin/noticeboard/', 'refresh');
        }
    ...
    function manage_profile($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'change_password') {
            ...
            $current_password = $this->db->get_where(
                'admin',
                array(
